...
### 2026-04-02 03:10-03:40 CST
- 完成：按 `XTR-SP-012` 实现信号决策回溯主链路透传与落盘。
  - `src/xtrader/runtime/core.py`：`decision_trace` 从策略结果透传到 `write_event_driven_outputs`，并同步产出 `artifacts/decision_trace.parquet` 与 `artifact_refs.decision_trace`。
  - `src/xtrader/backtests/offline_viewer.py`：离线资源初始化新增复制 `decision_trace_viewer.html`。
- 完成：扩展 report viewer 与新增独立查询页。
  - `src/xtrader/backtests/assets/offline_report_viewer.html`：新增顶部跳转按钮；交易行点击后展示进/出场决策链匹配摘要；接入 `decision_trace` 数据读取（chunk/parquet）。
  - `src/xtrader/backtests/assets/decision_trace_viewer.html`：新增独立查询工具，支持 `symbol + execution_time + action` 查询、候选列表、固定顺序详情区、JSON 结构化/原文切换与解析失败降级。
- 完成：补充测试覆盖与脚本输出。
  - `tests/unit/backtests/test_event_driven.py`、`tests/unit/runtime/test_runtime_v1.py`、`tests/unit/backtests/test_offline_viewer.py` 增补 `decision_trace` 相关断言。
  - `scripts/offline_report_viewer.py` 输出新增 `decision_trace_viewer_html_path`。
- 验证：
  - `PYTHONPATH=src pytest -q tests/unit/backtests/test_offline_viewer.py tests/unit/backtests/test_event_driven.py tests/unit/runtime/test_runtime_v1.py` -> `40 passed in 14.57s`
  - `python scripts/task_guard.py check XTR-SP-012` -> 通过（`✓ Spec & Validation check passed.`）
- 文档：`docs/03-delivery/validation/XTR-SP-012.md` 已回填执行日志（单测通过，手工 UI 与 smoke 实测待补证据）。

### 2026-04-02 22:55-23:15 CST
- 完成：按用户反馈调整 `XTR-SP-012` 页面交互。
  - `offline_report_viewer.html`：将“交易决策链”从页面常驻区改为 popup/modal，支持关闭（按钮、遮罩点击、ESC）。
  - `offline_report_viewer.html` -> `decision_trace_viewer.html`：在“在独立页面查看完整决策链”动作中新增 handoff（`postMessage`），直接传递当前 `symbol/execution_time/action` 与候选记录，避免二次选择 run 目录。
  - `decision_trace_viewer.html`：新增 handoff 接收与直接渲染逻辑；仍保留手工选择 run 目录查询的兜底流程。
- 完成：同步更新文档
  - `docs/03-delivery/specs/XTR-SP-012.md`（Design/Acceptance 增补 popup 与 handoff 约束）
  - `docs/03-delivery/validation/XTR-SP-012.md`（手工验证项与执行日志补充）
- 验证：
  - `PYTHONPATH=src pytest -q tests/unit/backtests/test_offline_viewer.py tests/unit/backtests/test_event_driven.py tests/unit/runtime/test_runtime_v1.py` -> `40 passed in 9.15s`
  - `python scripts/task_guard.py check XTR-SP-012` -> 通过（`✓ Spec & Validation check passed.`）

### 2026-04-02 23:16-23:22 CST
- 完成：修复 `decision_trace_viewer.html` 在 handoff 场景下 Run 目录控件“未选择任何文件”造成的可用性困惑。
  - 新增 `runDirInfo` 提示区，明确显示“当前数据来源”。
  - handoff 场景显示：`Report Viewer 直连（run_id=...）`，无需二次选择目录。
  - 手动选择目录场景显示：目录名、文件数与加载后 `run_id`。
- 验证：
  - `PYTHONPATH=src pytest -q tests/unit/backtests/test_offline_viewer.py tests/unit/backtests/test_event_driven.py tests/unit/runtime/test_runtime_v1.py` -> `40 passed in 8.98s`
  - `PYTHONPATH=src python scripts/offline_report_viewer.py init --output reports/backtests/viewer` -> 成功生成最新页面资源。

### 2026-04-02 23:23-23:35 CST
- 完成：按用户确认落地 `offline_report_viewer` 的 Trade Ledger 可检索优化（v1）。
  - 新增筛选工具条：关键词、方向、PnL（类型+区间）、开仓时间范围、排序。
  - 新增查找辅助：页码跳转、时间定位到最接近交易。
  - 保持联动主链路不变：筛选后点击交易仍触发 K 线/资金曲线/时间线聚焦；筛选导致已选交易失效时自动回退 global。
- 完成：同步文档
  - `docs/03-delivery/specs/XTR-SP-012.md` 增补 Trade Ledger v1 设计与验收条款。
  - `docs/03-delivery/validation/XTR-SP-012.md` 增补手工验证项与执行日志。
- 验证：
  - `PYTHONPATH=src pytest -q tests/unit/backtests/test_offline_viewer.py tests/unit/backtests/test_event_driven.py tests/unit/runtime/test_runtime_v1.py` -> `40 passed in 11.11s`
  - `python scripts/task_guard.py check XTR-SP-012` -> 通过（`✓ Spec & Validation check passed.`）
  - `PYTHONPATH=src python scripts/offline_report_viewer.py init --output reports/backtests/viewer` -> 成功生成最新 viewer 资源。

### 2026-04-02 23:36-23:48 CST
- 完成：继续优化 `offline_report_viewer` 的 Trade Ledger 时间输入与布局。
  - 将 `起始时间/结束时间/时间定位` 从文本框改为 `datetime-local` picker，固定输入格式。
  - 重排筛选工具条为“Label + Control 同组”的网格布局，避免换行时标签与控件错位。
  - 时间定位提示文案改为 picker 语义，减少手动格式输入错误。
- 完成：同步更新文档
  - `docs/03-delivery/specs/XTR-SP-012.md`：补充 datetime picker 与布局约束。
  - `docs/03-delivery/validation/XTR-SP-012.md`：补充验证目标与执行日志。
- 验证：
  - `PYTHONPATH=src pytest -q tests/unit/backtests/test_offline_viewer.py tests/unit/backtests/test_event_driven.py tests/unit/runtime/test_runtime_v1.py` -> `40 passed in 9.68s`
  - `python scripts/task_guard.py check XTR-SP-012` -> 通过（`✓ Spec & Validation check passed.`）
  - `PYTHONPATH=src python scripts/offline_report_viewer.py init --output reports/backtests/viewer` -> 成功生成最新 viewer 资源。
